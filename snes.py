...
def computeUtilities(fitnesses):
    L = len(fitnesses)
    ranks = zeros_like(fitnesses) #return an array of zeros with the same shape
    l = list(zip(fitnesses, range(L))) # return a list (fitnesses, order)
    l.sort() # ascending fitnesses?
    for i, (_, j) in enumerate(l): # store rank from fitnesses in array
        ranks[j] = i
    # smooth reshaping
...
class SNES():
    ...
    def tell(self, asked, fitnesses):
        ...
        # print fitness score should be loss
