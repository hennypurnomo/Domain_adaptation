# this code is from Lyu Chaofan 1706987, without any changes
...
if __name__ == '__main__':
    import cv2
    image_data = cv2.imread('./Original_images/amazon/images/back_pack/frame_0001.jpg')
    #print (image_data)
    labels = {'printer': 21, 
              'projector': 22, 
              'file_cabinet': 9, 
    ...
    test = dataSet()
    test.upData(image_data, labels['back_pack'], labels)
    test.upData(image_data, labels['back_pack'], labels)
    test.upData(image_data, labels['back_pack'], labels)
    test.upData(image_data, labels['back_pack'], labels)
    test.sHape()
